...
int main(void)
{
  ...
  // DBG_UART_Init();
  // printf("[boot] printf path OK\r\n");  

  //#######################################################################
// --- Force-init USART1 for ST-Link VCP (115200 8N1) ---
...
if (HAL_UART_Init(&hDiscoUart) != HAL_OK) {
  while (1) { /* UART init failed — optional blink here */ }
}

// Unbuffer stdout so each printf appears immediately
setvbuf(stdout, NULL, _IONBF, 0);

// PROBE: must show up even if printf isn’t routed yet
const char *probe = "\r\n[probe] USART1 TX OK (PB6/PB7 @115200)\r\n";
HAL_UART_Transmit(&hDiscoUart, (uint8_t*)probe, strlen(probe), HAL_MAX_DELAY);

// Your TERMOUT macro already maps to printf
TERMOUT("[boot] printf path OK?\r\n");
...
void HAL_UART_MspInit(UART_HandleTypeDef* huart)
{
  if (huart->Instance == USART1)
  {
    ...
    // B-L475E-IOT01 VCP: PB6 = USART1_TX, PB7 = USART1_RX, AF7
