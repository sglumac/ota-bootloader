...
{
  TERMOUT("Gonna Jump to Application\n");
  void (*app_reset_handler)(void) = (void*)(*((volatile uint32_t*) (0x08080000 + 4U)));
  //__set_MSP(*(volatile uint32_t*) 0x08040000);
  // Turn OFF the Green Led to tell the user that Bootloader is not running
  //HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_RESET );    //Green LED OFF

  /* Reset the Clock */
  //HAL_RCCprintf_DeInit();
  for(int i=0; i<8; i++) NVIC->ICPR[i] = 0xFFFFFFFF; // Clear pending interrupts
  uwTick = 0;
  __disable_irq();
...
HAL_StatusTypeDef Flash_WriteBank2Page(uint32_t offset, uint8_t *data)
{
    ...
    uint32_t pageAddress = 0x08080000U + offset;  // Bank 2 base + offset
    uint32_t i;
    ...
    eraseInitStruct.Page      = (offset / (FLASH_PAGE_SIZE/4)); // Page index in Bank 2
          ...
          if (ret == WIFI_STATUS_OK) {
              if (Datalen > 0) {
				  ...
				  if (RxData[0] == 0) {
					  ...
					  //Flash_WriteBank2Page(offset,&RxData[1]);
